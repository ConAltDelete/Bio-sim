...
from .visuals import set_param, string2map
    ...
    def add_population(self, population: dict):
        ...
        for coord in population:
            ...
            for animal in population[coord]:
                if animal["species"].lower() == "herbevore":
                    cell.herb_default.append(
                        herbavor(a=animal["age"], w=animal["weight"]))
                elif animal["species"].lower() == "carnivore":
                    cell.carn_default.append(
                        preditor(a=animal["age"], w=animal["weight"]))
                else:
                    raise ValueError(
                        "Got '{}'; needs 'herbevore' or 'carnivore'".format(animal["species"]))
